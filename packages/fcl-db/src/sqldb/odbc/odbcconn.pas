...
begin
  ...
  try
    ...
    wbuf:=ConnectionString;
    CheckResult(
      SQLDriverConnectW(FDBCHandle,            // the ODBC connection handle
                        nil,                   // no parent window (would be required for prompts)
                        PWideChar(wbuf),       // the connection string
                        Length(wbuf),          // connection string length
                        nil,                   // buffer for storing the completed connection string (unused)
