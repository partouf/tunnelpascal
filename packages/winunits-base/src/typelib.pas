...
{
  Contains six bug fixes for
  https://gitlab.com/freepascal.org/lazarus/ccr/-/issues/39012
  TypeLib importer ignores the [optional] and [defaultvalue()] parameter attributes
  https://gitlab.com/freepascal.org/lazarus/ccr/-/issues/39013
  TypeLib importer ignores all [in] parameters of properties except the first parameter
  https://gitlab.com/freepascal.org/lazarus/ccr/-/issues/39014
  TypeLib importer reintroduces all members of IUnknown and IDispatch in descendant interfaces
  https://gitlab.com/freepascal.org/lazarus/ccr/-/issues/39016
  TypeLib importer imports enum as LongWord instead of Integer
  https://gitlab.com/freepascal.org/fpc/source/-/issues/39320
  Compiler error "It is not possible to define a default value for a parameter of type OleVariant"
  https://gitlab.com/freepascal.org/fpc/source/-/issues/39340
  fpc refuses to compile more than one default property with same name

  Two following bugs require changes to the fpc compiler:
  https://gitlab.com/freepascal.org/lazarus/ccr/-/issues/39015
  TypeLib importer never makes a property default if it has no [in] parameters
  https://gitlab.com/freepascal.org/fpc/source/-/issues/39358
  type helper for dispinterface fails to compile property
}
...
var
  ...
  cnt:LongWord;
  TD: TYPEDESC;
  bPropHasParam,bIsFunction,bParamByRef,bDispWritten:boolean;
  VD: lpVARDESC;
  aPropertyDefs:array of TPropertyDef;
  Propertycnt,iType,usedProps,optParams,reqOverloads:integer;
  Modifier: string;
  vType: TVarType;
          ...
          begin
            ...
            begin
              if AnsiEndsText('OleVariant', TypeToString(TI,FD^.lprgelemdescParam[k].tdesc)) then // param is VT_VARIANT or was non-automatable and got converted into OleVariant. remember the desired default value for overloading
                ...
                sPropParam:=sPropParam+format(' = %s',[StringReplace(VarToWideStr(Variant(FD^.lprgelemdescParam[k].paramdesc.pparamdescex^.varDefaultValue)),',','.',[])]);
              optParams:=0; // reset counter in case a defaultvalue parameter comes after an optional parameter (which is invalid according to IDL)
            end
            else if ((FD^.lprgelemdescParam[k].paramdesc.wParamFlags and PARAMFLAG_FOPT) <> 0)
            ...
            begin
              ...
              optParams:=0; // reset counter in case a normal parameter comes after an optional parameter (which is invalid according to IDL)
            if (((FD^.lprgelemdescParam[k].paramdesc.wParamFlags and PARAMFLAG_FHASDEFAULT) <> 0) and
              (FD^.lprgelemdescParam[k].paramdesc.pparamdescex^.varDefaultValue.vt = VT_VARIANT))
              or
