      ...
      begin
        case n.nodetype of
          ...
          raisen, tryfinallyn:
            ...
            res:=foreachnodestatic(ttertiarynode(n).third,f,arg) or res;
          tempcreaten:
            { temp. initialization code }
            if assigned(ttempcreatenode(n).tempinfo^.tempinitcode) then
              res:=foreachnodestatic(ttempcreatenode(n).tempinfo^.tempinitcode,f,arg) or res;
          casen:
...
{$endif ARM}
    ...
    function donodecount(var n: tnode; arg: pointer): foreachnoderesult;
      begin
        PInteger(arg)^:=PInteger(arg)^-1;
        if PInteger(arg)^>0 then
          result:=fen_false
        else
