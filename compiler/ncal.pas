...
interface
    ...
    type
       tcallnodeflag = (
         ...
         cnf_do_pure,
         cnf_pure_abort          { could not successfully analyse pure function call (but not in an erroneous way) }
       );
       ...
       protected
          ...
          class function HasUnconditionalBreak(n: tstatementnode): tnode; static;
          function pure_strip_dead_labels(var n: tnode; arg: pointer): foreachnoderesult;
          function pure_convert_break_continue(var n: tnode; arg: pointer): foreachnoderesult;
          function pure_loop_unroll(var n: tnode; arg: pointer): foreachnoderesult;
          function pure_expand_nested_calls(var n: tnode; arg: pointer): foreachnoderesult;

          procedure WritePureNodeLimitWarning;
       protected
          { inlining support }
...
{$endif DEBUG_NODE_XML}
    ...
    const
      ...
      { track stack depth of pure function analysis }
      purelevel : longint = 0;

    type
      PBooleanArg = ^TBooleanArg;
      TBooleanArg = record
        Flag: Boolean;
        Error: Boolean;
      end;

      PBreakContinueInfo = ^TBreakContinueInfo;
      TBreakContinueInfo = record
        BreakFound: Boolean;
        BreakLabel: TNode;
...
implementation
    ...
    uses
      ...
      htypechk,pass_1,
      ncnv,nflw,nld,ninl,nmat,nadd,ncon,nmem,nset,nobjc,
      pgenutil,
      ngenutil,objcutil,aasmcnst,
      procinfo,cpuinfo,
      wpobase,
      { These units are used during the analysis of pure functions }
      optcse,optconstprop,optdfa,optdeadstore,psub;
    ...
    const
      PURE_NODE_LIMIT = 16384;
      PURE_DEPTH_LIMIT = 64;
      PURE_ITERATION_LIMIT = 256;
...
{$endif jvm}
    ...
    procedure tcallnode.check_purity;
      var
        st   : tsymtable;
      begin
        { Can we make the procedure pure? }
        if (po_pure in procdefinition.procoptions) and
           not (cnf_pure_abort in callnodeflags) and
           (procdefinition.typ=procdef) and
           tprocdef(procdefinition).has_purityinfo then
          begin
            { Prevent stack overflows from malicious or overly complex code }
            if purelevel >= PURE_DEPTH_LIMIT then
              begin
                WritePureNodeLimitWarning;
                Exit;
              end;

            include(callnodeflags,cnf_do_pure);
            { Check if we can make the call pure when it references proc/var
              that are not in the globally available }
            st:=procdefinition.owner;
            while (st.symtabletype in [ObjectSymtable,recordsymtable]) do
              st:=st.defowner.owner;
            if not(tf_supports_hidden_symbols in target_info.flags) and
               (pi_uses_static_symtable in tprocdef(procdefinition).purityinfo^.flags) and
               (st.symtabletype=globalsymtable) and
               (not st.iscurrentunit) then
              begin
                Comment(V_lineinfo+V_Debug,'Not attempting pure for "'+tprocdef(procdefinition).procsym.realname+'", references private symbols from other unit');
                exclude(callnodeflags,cnf_do_pure);
                Include(callnodeflags,cnf_pure_abort);
              end;
          end;
      end;
         ...
         try
           ...
           if (cs_opt_remove_empty_proc in current_settings.optimizerswitches) and
             ...
             ((tprocdef(procdefinition).procoptions-[po_none,po_classmethod,po_staticmethod,
               ...
               po_inline,po_pure,po_compilerproc,po_has_importdll,po_has_importname,po_kylixlocal,po_dispid,po_delphi_nested_cc,
           ...
           if (procdefinition.typ=procdef) and
             ...
             begin
               ...
               { Check if the call can take advantage of the callee being pure,
                 sets the cnf_do_pure flag }
               check_purity;

               if (cnf_do_pure in callnodeflags) and not (cnf_pure_abort in callnodeflags) then
                 result:=pass1_pure
               else
                 result:=nil;
           ...
           if not (tnf_error in transientflags) then
             begin
    ...
    function tcallnode.pass1_normal : tnode;
      begin
         ...
         { If the parameters were pure functions that have since been resolved,
           we may have a case where we can evaluate this function as a pure
           function whereas we couldn't before (this might not be as efficient
           as it could be because of extra assigned temps) }

         { Check that po_pure is actually set though - it gets removed in error
           conditions }
         if (cnf_do_pure in callnodeflags) and
           not (cnf_pure_abort in callnodeflags) and
           (po_pure in procdefinition.procoptions) then
           begin
             result:=pass1_pure;
             if Assigned(Result) then
               Exit;
...
**************************************************************************}
    ...
    { Force range and overflow checks }
    function forcerangeoverflowchecks(var n: tnode; arg: pointer): foreachnoderesult;
      begin
        Result := fen_true;
        Include(n.localswitches, cs_check_range);
        Include(n.localswitches, cs_check_overflow);
      end;
    ...
    function tcallnode.pass1_inline:tnode;
      ...
      begin
        ...
        if body.nodetype=blockn then
          include(body.flags,nf_block_with_exit);
...
{$endif DEBUGINLINE}
    ...
    class function tcallnode.HasUnconditionalBreak(n: tstatementnode): tnode;
      begin
        Result := nil;
        while Assigned(n) do
          begin
            case n.statement.nodetype of
              blockn:
                begin
                  { Evaluate nested statement blocks }
                  Result := HasUnconditionalBreak(TStatementNode(TBlockNode(n.statement).statements));
                  if Assigned(Result) then
                    Exit;
                end;
              ...
              statementn:
                begin
                  { A statement in a statement may happen sometimes }
                  Result := HasUnconditionalBreak(TStatementNode(TStatementNode(n.statement).statement));
                  if Assigned(Result) then
                    Exit;
                end;
    ...
    function check_label_references(var n: tnode; arg: pointer): foreachnoderesult;
      begin
        if (n.nodetype = goton) and (TGotoNode(n).labelnode = TNode(arg)) then
          Result := fen_norecurse_true
        else
          Result := fen_false;
      end;
    ...
    function tcallnode.pure_strip_dead_labels(var n: tnode; arg: pointer): foreachnoderesult;
      begin
        Result := fen_false;
        if (n.nodetype = labeln) and
          (nf_internal in n.Flags) and
          not TLabelNode(n).labsym.used and
          not Assigned(TLabelNode(n).labsym.Owner) and
          not foreachnodestatic(TNode(arg), @check_label_references, n) then
          begin
            { Label is not referenced - delete }
            n.Free;
    ...
    function tcallnode.pure_convert_break_continue(var n: tnode; arg: pointer): foreachnoderesult;
      begin
        case n.nodetype of
          breakn:
            begin
              PBreakContinueInfo(arg)^.BreakFound := True;
              n := cgotonode.create(TLabelNode(PBreakContinueInfo(arg)^.BreakLabel).labsym);
              Include(n.Flags, nf_internal);
              TGotoNode(n).labelnode := TLabelNode(PBreakContinueInfo(arg)^.BreakLabel);
              result := fen_true;
            end;
          continuen:
            begin
              PBreakContinueInfo(arg)^.ContinueFound := True;
              n := cgotonode.create(TLabelNode(PBreakContinueInfo(arg)^.ContinueLabel).labsym);
              Include(n.Flags, nf_internal);
              TGotoNode(n).labelnode := TLabelNode(PBreakContinueInfo(arg)^.ContinueLabel);
              result := fen_true;
            end;
          else
        end;
    ...
    function tcallnode.pure_loop_unroll(var n: tnode; arg: pointer): foreachnoderesult;
      var
        IterationsM1, First, Last, Current: TConstExprInt;
        X, UnrollLimit, NC: LongWord;
        TempNode, NewNode, RootNode, RightNodes: TNode;
        StatementNode: TStatementNode;
        BreakContinueInfo: TBreakContinueInfo;
        TempSym: TLabelSym;
      begin
        if PBooleanArg(arg)^.Error then
        ...
        case n.nodetype of
          forn:
            begin
              { Limit the number of iterations we unroll in order to avoid
                exploding the node tree too much and either hitting the node
                limit or an out of memory error }
              NC := Max(node_count(tfornode(n).t2), 1);

              UnrollLimit := ((1023 + NC) div NC) - 1; { Ceil(1024 / NC) - 1}
              if UnrollLimit = 0 then
                InternalError(2024050201);

              { Simplify the "from" and "to" nodes. (This has to be done after
                constant propagation, but before the re-run of pass 1, so actual
                parameters of pure functions can be converted into constants
                where possible) }
              if dopuresimplify(tfornode(n).right) then
                PBooleanArg(arg)^.Flag := True; { "changed" flag }
              if dopuresimplify(tfornode(n).t1) then
                PBooleanArg(arg)^.Flag := True; { "changed" flag }

              if
                { Any ordinal constant type is valid }
                (tfornode(n).right.nodetype = ordconstn) and
                (tfornode(n).t1.nodetype = ordconstn) then
                begin
                  { Check for an empty for-loop }
                  if not Assigned(tfornode(n).t2) or (tfornode(n).t2.nodetype = nothingn) then
                    begin

                      if Assigned(tfornode(n).t2) then
                        begin
                          { Reuse the nothing node to save effort }
                          NewNode := tfornode(n).t2;
                          tfornode(n).t2 := nil;
                        end
                      else
                        NewNode := cnothingnode.create();
                  ...
                  if (lnf_backward in tfornode(n).loopflags) then
                    begin
                      First := tordconstnode(tfornode(n).t1).value;
                      Last := tordconstnode(tfornode(n).right).value;
                    end
                  else
                    begin
                      Last := tordconstnode(tfornode(n).t1).value;
                      First := tordconstnode(tfornode(n).right).value;
                    end;
                  ...
                  IterationsM1 := Last - First;

                  if ((IterationsM1 * NC) >= PURE_NODE_LIMIT) and (IterationsM1 < UnrollLimit) then
                    begin
                      { Too many nodes }

                      WritePureNodeLimitWarning;
                      PBooleanArg(arg)^.Error := True;
                      Result := fen_norecurse_true;
                      Exit;
                    end;

                  { Prepare for conversion of Break nodes }
                  BreakContinueInfo.BreakFound := False;
                  TempSym := CLabelSym.create('$' + hexstr(Pointer(Self)) + hexstr(Pointer(n)) + '_break');
                  BreakContinueInfo.BreakLabel := CLabelNode.create(nil, TempSym);
                  Include(BreakContinueInfo.BreakLabel.Flags, nf_internal);
                  TempSym.code := Pointer(BreakContinueInfo.BreakLabel);

                  RootNode := internalstatements(StatementNode);

                  { Single iteration - simplest case }
                  if IterationsM1 = 0 then
                    begin
                      { Prepare for conversion of Continue nodes }
                      BreakContinueInfo.ContinueFound := False;
                      TempSym := CLabelSym.create('$' + hexstr(Pointer(Self)) + hexstr(Pointer(n)) + '_0_continue');
                      BreakContinueInfo.ContinueLabel := CLabelNode.create(nil, TempSym);
                      Include(BreakContinueInfo.ContinueLabel.Flags, nf_internal);
                      TempSym.code := Pointer(BreakContinueInfo.ContinueLabel);
                      ...
                      AddStatement(StatementNode, tfornode(n).t2);

                      foreachnode(pm_postprocess, tfornode(n).t2, @pure_convert_break_continue, @BreakContinueInfo);

                      tfornode(n).left := nil;
                      tfornode(n).right := nil;
                      tfornode(n).t2 := nil;

                      if BreakContinueInfo.ContinueFound then
                        addstatement(StatementNode, BreakContinueInfo.ContinueLabel)
                      else
                        BreakContinueInfo.ContinueLabel.Free;
                    end
                  else
                    begin
                      if IterationsM1 >= PURE_NODE_LIMIT then
                        begin
                          { This shouldn't be reached, but it's feasible that node_count could return zero }
                          WritePureNodeLimitWarning;
                          PBooleanArg(arg)^.Error := True;
                          Result := fen_norecurse_true;
                          Exit;
                      ...
                      UnrollLimit := Min(UnrollLimit, LongWord(IterationsM1.uvalue));

                      for X := 0 to UnrollLimit do
                        begin
                          if IterationsM1 >= UnrollLimit then
                            IterationsM1 := UnrollLimit;

                          { Prepare for conversion of Continue nodes }
                          BreakContinueInfo.ContinueFound := False;
                          TempSym := CLabelSym.create('$' + hexstr(Pointer(Self)) + hexstr(Pointer(n)) + '_' + tostr(X) + '_continue');
                          BreakContinueInfo.ContinueLabel := CLabelNode.create(nil, TempSym);
                          Include(BreakContinueInfo.ContinueLabel.Flags, nf_internal);
                          TempSym.code := Pointer(BreakContinueInfo.ContinueLabel);

                          NewNode := tfornode(n).t2.getcopy();

                          AddStatement(StatementNode,
                            cassignmentnode.create(
                              tfornode(n).left.getcopy(),
                              cordconstnode.create(Current, tfornode(n).right.resultdef, False)
                            )
                          );

                          foreachnode(pm_postprocess, NewNode, @pure_convert_break_continue, @BreakContinueInfo);

                          addstatement(StatementNode, NewNode);
                      ...
                      if UnrollLimit < LongWord(IterationsM1.uvalue) then
                        begin
                          { We only unwrapped part of the loop, so attach the
                            remainder to the end }

                          { Avoid duplicating the entirety of the for-loop for
                            performance reasons }
                          TempNode := tfornode(n).t2;
                          tfornode(n).t2 := nil;

                          NewNode := n.getcopy();
                          tfornode(NewNode).t2 := TempNode;
                          tordconstnode(tfornode(NewNode).right).value := Current;
                          addstatement(StatementNode, NewNode);
                        end;
                    end;

                  { Value of the counter variable after loop exits }
                  if (lnf_backward in tfornode(n).loopflags) then
                    Last := tordconstnode(tfornode(n).t1).value - 1
                  else
                    Last := tordconstnode(tfornode(n).t1).value + 1;
                  ...
                  addstatement(StatementNode,
                    cassignmentnode.create(
                      tfornode(n).left.getcopy,
                      cordconstnode.create(Last, tordconstnode(tfornode(n).t1).resultdef, False)
                    )
                  );

                  if BreakContinueInfo.BreakFound then
                    addstatement(StatementNode, BreakContinueInfo.BreakLabel)
                  else
                    BreakContinueInfo.BreakLabel.Free;
                ...
                begin
                  { Iteration count non-deterministic }
                  MessagePos2(n.fileinfo,parser_w_pure_ineligible,tprocdef(procdefinition).procsym.realname,'non-deterministic for loop');
          ...
          whilerepeatn:
            begin
              RightNodes := TWhileRepeatNode(n).right.getcopy();

              BreakContinueInfo.BreakFound := False;
              TempSym := CLabelSym.create('$' + hexstr(Pointer(Self)) + hexstr(Pointer(n)) + hexstr(Pointer(RightNodes)) + '_break');
              BreakContinueInfo.BreakLabel := CLabelNode.create(nil, TempSym);
              Include(BreakContinueInfo.BreakLabel.Flags, nf_internal);
              TempSym.code := Pointer(BreakContinueInfo.BreakLabel);

              BreakContinueInfo.ContinueFound := False;
              TempSym := CLabelSym.create('$' + hexstr(Pointer(Self)) + hexstr(Pointer(n)) + hexstr(Pointer(RightNodes)) + '_continue');
              BreakContinueInfo.ContinueLabel := CLabelNode.create(nil, TempSym);
              Include(BreakContinueInfo.ContinueLabel.Flags, nf_internal);
              TempSym.code := Pointer(BreakContinueInfo.ContinueLabel);

              foreachnode(pm_postprocess, RightNodes, @pure_convert_break_continue, @BreakContinueInfo);

              if (lnf_testatbegin in TWhileRepeatNode(n).loopflags) then
                begin
                  NewNode := TWhileRepeatNode(n).left.getcopy();
                  if (lnf_checknegate in TWhileRepeatNode(n).loopflags) then
                    NewNode := cnotnode.create(NewNode);

                  RootNode := internalstatements(StatementNode);

                  addstatement(StatementNode, RightNodes);

                  if BreakContinueInfo.ContinueFound then
                    addstatement(StatementNode, BreakContinueInfo.ContinueLabel);

                  { Save some time and memory by re-using the right nodes
                    rather than duplicating them. }
                  RightNodes := TWhileRepeatNode(n).right;
                  TWhileRepeatNode(n).right := nil;

                  TempNode := TWhileRepeatNode(n).getcopy();
                  TWhileRepeatNode(TempNode).right := RightNodes;

                  addstatement(StatementNode, TempNode);

                  RootNode := cifnode.create_internal(
                    NewNode,
                    RootNode,
                    nil
                  );
                ...
                begin
                  ...
                  { Convert the repeat...until into an equivalent while loop }
                  NewNode := TWhileRepeatNode(n).getcopy();
                  TWhileRepeatNode(NewNode).right := RightNodes;
                  if (lnf_checknegate in TWhileRepeatNode(NewNode).loopflags) then
                    begin
                      TWhileRepeatNode(NewNode).left := cnotnode.create(TWhileRepeatNode(NewNode).left);
                      Exclude(TWhileRepeatNode(NewNode).loopflags, lnf_checknegate);
                    end;

                  Include(TWhileRepeatNode(NewNode).loopflags, lnf_testatbegin);
                  addstatement(StatementNode, NewNode);
                end;
              ...
              if not BreakContinueInfo.BreakFound then
                BreakContinueInfo.BreakLabel.Free;

              if not BreakContinueInfo.ContinueFound then
                BreakContinueInfo.ContinueLabel.Free;

              PBooleanArg(arg)^.Flag := True;

              { If we don't set 'norecurse', then the embedded while loop will
                also be expanded, and then the embedded while loop in that one
                will be expanded ad infinitum }
              Result := fen_norecurse_false;
            end;
          else
            ;
    ...
    function tcallnode.pure_expand_nested_calls(var n: tnode; arg: pointer): foreachnoderesult;
      var
        oldnode: tnode;
      begin
        Result := fen_false;
        if n.nodetype = calln then
          begin
            node_reset_flags(n,[],[tnf_pass1_done]);
            oldnode := n;
            if do_firstpass(n) then
              begin
                { Error condition }
                PBooleanArg(arg)^.Error := True;
                Result := fen_norecurse_true;
              end
            else if n <> oldnode then
              { The call node was transmuted in some way, so mark the change }
              PBooleanArg(arg)^.Flag := True
            else if (cnf_pure_abort in TCallNode(n).callnodeflags) then
              begin
                { Function couldn't be successfully expanded }
                Result := fen_norecurse_true;
              end;
          end;
      end;
    ...
    function tcallnode.pass1_pure:tnode;
      var
        X : Integer;
        n,
        resultnode,
        outputbody,
        body : tnode;
        outputs: TFPObjectList;
        ResultCovered,
        ParameterFound : Boolean;
        outputstatements,
        purestatements : tstatementnode;
        para : tcallparanode;
        pureblock: tnode;
        changed, dfachanged, propchanged: boolean;
        old_switches,
        deadstore_switches,
        pure_local_switches: tlocalswitches;
        forchanged: TBooleanArg;
        PureDFABuilder: TDFABuilder;
        old_currpi: TProcInfo;
        old_optimizerswitches: TOptimizerSwitches;
        run: Longint;
        ErrorCountBefore, ErrorCountAfter: LongInt;

        procedure PureCleanup;
          begin
            current_procinfo.Free;
            current_procinfo := old_currpi;
            current_settings.localswitches := old_switches;
            current_settings.optimizerswitches := old_optimizerswitches;
            PureDFABuilder.Free;
            outputs.Free;
            pureblock.free;
            dec(purelevel);
        ...
        procedure NonTrivial;
          begin
            MessagePos2(fileinfo,parser_w_pure_ineligible,tprocdef(procdefinition).procsym.realname,'analysis did not produce a simple assignment to the outputs');
            Exclude(procdefinition.procoptions, po_pure);
{$ifdef DEBUG_PURE}
            WriteLn('-- ABORT PURITY ANALYSIS --');
            WriteLn('End result was not a simple assignment to the outputs');
{$endif DEBUG_PURE}
          end;

        function SimplifyBlock: Boolean;
          var
            expandchanged: TBooleanArg;
          begin
            Result := False;
            ...
            repeat
              ...
              { Run dopuresimplify to eliminate conditional blocks that may have
                nodes that are now erroneous (e.g. divisions by zero that are behind
                a check to see if the divisor is non-zero) }
              changed := changed or dopuresimplify(tnode(pureblock));

              ErrorCountAfter := ErrorCount();

              { If error counts differ, an error of some kind occurred during the
                analysis.  Dropping out now will potentially prevent an undesirable
                cascade of error messages }
              if (ErrorCountAfter <> ErrorCountBefore) then
                begin
                  { Prevent this error from appearing multiple times (such as in a recursive pure function) }
                  if not codegenerror then
                    begin
                      MessagePos2(fileinfo,parser_e_pure_analysis_error, tostr(ErrorCountAfter - ErrorCountBefore), tprocdef(procdefinition).procsym.realname);
...
{$ifdef DEBUG_PURE}
                  WriteLn('-- ABORT PURITY ANALYSIS --');
                  WriteLn('Error occurred during analysis');
{$endif DEBUG_PURE}
                  Exit;
                end;

              if not (po_pure in procdefinition.procoptions) then
                begin
                  PureCleanup;
{$ifdef DEBUG_PURE}
                  WriteLn('-- ABORT PURITY ANALYSIS --');
                  WriteLn('Function is no longer considered pure');
{$endif DEBUG_PURE}
                  Exit;
                end;

              { Attempt to expand nested pure and inline procedure calls by selectively calling
                firstpass only on call nodes.  Note, use pm_preprocess so function parameters
                that are themselves functions are processed first }
              expandchanged.Flag := false;
              expandchanged.Error := false;

              if foreachnode(pm_preprocess, pureblock, @pure_expand_nested_calls, @expandchanged) then
                begin
                  PureCleanup;
                  Exclude(callnodeflags, cnf_do_pure);
                  Include(callnodeflags, cnf_pure_abort);

                  { If it returned true, either something failed to expand (i.e.
                    a pure function couldn't be successfully analysed) or we
                    got some kind of error }
                  if expandchanged.Error then
                    begin
                      Include(transientflags, tnf_error);
                      codegenerror:=true;
{$ifdef DEBUG_PURE}
                      WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
                      WriteLn('Error while expanding nested calls');
{$endif DEBUG_PURE}
                    end
                  else
                    begin
{$ifdef DEBUG_PURE}
                      WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
                      WriteLn('Could not expand nested subroutine call');
{$endif DEBUG_PURE}
                    end;
                  Exit;
                end;

              changed := expandchanged.Flag or changed;

            until not expandchanged.Flag;
      ...
      begin
        inc(purelevel);
        outputs := TFPObjectList.Create(False);
        result := nil;
        PureDFABuilder := nil;
        run := 0;

        if not(assigned(tprocdef(procdefinition).purityinfo) and
               assigned(tprocdef(procdefinition).purityinfo^.code)) then
          internalerror(2022051601);

        old_optimizerswitches := current_settings.optimizerswitches;

        { We need all of the optimisations turned to full for pure functions to
        ...
        old_switches := current_settings.localswitches;

        old_currpi := current_procinfo;

        pureblock:=internalstatements(purestatements);

        { Add the parameters }
        para:=tcallparanode(left);
        while assigned(para) do
          begin
            case para.parasym.varspez of
              vs_value,vs_const:
                begin
                  { Actual parameters must be constants }
                  if not is_constnode(para.left) then
                    begin
                      current_procinfo := nil; { Prevent PureCleanup from freeing it }
                      PureCleanup;
{$ifdef DEBUG_PURE}
                      WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
                      WriteLn('Actual parameter is not a constant');
{$endif DEBUG_PURE}
                      Exit;
                    end;

                  { Prevents syntax errors from appearing in the code analysis }
                  para.parasym.varspez := vs_value;

                  addstatement(
                    purestatements,
                    cassignmentnode.create(
                      cloadnode.create(para.parasym, procdefinition.parast),
                      para.left.getcopy
                    )
                  );
                end;
              ...
              vs_var:
                begin
                  if not (vo_is_funcret in para.parasym.varoptions) then
                    { Function should not be marked as pure }
                    internalerror(2022051611);

                  para.parasym.varspez := vs_out;
                  outputs.Add(para);
                end;
              else
                { Function should not be marked as pure }
                internalerror(2022051610);
            end;

            para:=tcallparanode(para.right);
          end;

        { Create a new temporary current_procinfo }
        current_procinfo:=cprocinfo.create(nil);
        current_procinfo.procdef:=tprocdef(procdefinition);

        { create a copy of the body and append it to the parameter loads }
        body:=tprocdef(procdefinition).purityinfo^.code.getcopy;

        pure_local_switches := body.localswitches;
        deadstore_switches := body.localswitches - [cs_check_range, cs_check_overflow];

        foreachnodestatic(pm_postprocess,body,@removeusercodeflag,nil);

        addstatement(purestatements,body);

        { Switch to the switches active when the pure function was first implemented }
        current_settings.localswitches := pure_local_switches;

        typecheckpass(tnode(pureblock));

        { Force range checking on all constants }
        foreachnodestatic(pm_postprocess, body, @forcerangeoverflowchecks, nil);

        { Do a constant propagation first }
        do_optconstpropagate(pureblock, changed, True);

        { And then simplify the result }
        if changed then
          dopuresimplify(tnode(pureblock));

        PureDFABuilder := TDFABuilder.Create;

        repeat
          Inc(run);
          if run > PURE_ITERATION_LIMIT then
            begin
              { Possible infinite loop, or procedure is too complex }
              WritePureNodeLimitWarning;
              PureCleanup;
{$ifdef DEBUG_PURE}
              WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
...
{$ifdef DEBUG_PURE}
          WriteLn('---------------------------------');
          WriteLn('Running purity analysis... pass ', run);
          WriteLn('---------------------------------');
  {$ifdef DEBUG_NODE_XML}
          { Has better formatting }
          pureblock.XMLPrintNodeTree(StdOut);
  {$else DEBUG_NODE_XML}
          pureblock.printnodetree(StdOut);
  {$endif DEBUG_NODE_XML}
{$endif DEBUG_PURE}

          changed := false;
          forchanged.Flag := false;
          forchanged.Error := false;

          ErrorCountBefore := ErrorCount();

          { Unroll the for loops }
          if foreachnode(pm_postprocess, pureblock, @pure_loop_unroll, @forchanged) then
            begin
              { Something couldn't be safely unrolled }
              PureCleanup;
{$ifdef DEBUG_PURE}
              WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
              WriteLn('Could not safely unroll loop');
{$endif DEBUG_PURE}
              Exit;
            end;
...
{$endif DEBUG_PURE}
          ...
          if not SimplifyBlock then
            { An error occurred }
            Exit;

          changed := changed or forchanged.Flag;

{$ifdef DEBUG_PURE}
          if changed then
            begin
              WriteLn('---------------------------------');
              WriteLn('Loop and call expansion complete.');
              WriteLn('---------------------------------');
  {$ifdef DEBUG_NODE_XML}
              { Has better formatting }
              pureblock.XMLPrintNodeTree(StdOut);
...
{$endif DEBUG_PURE}
          ...
          if changed or (run = 1) then { Always run constant propagation on the first run }
            repeat
              do_optconstpropagate(pureblock, propchanged, True);

              dfachanged := False;

              { Disable range and overflow checking for the moment since this blocks
                deadstore elimination }
              current_settings.localswitches := deadstore_switches;

              PureDFABuilder.redodfainfo(pureblock);
              do_optdeadstoreelim(pureblock, dfachanged);

              current_settings.localswitches := pure_local_switches;

              { If DFA changed things, simplify the block to help another round
              ...
              { Changed gets overwritten by SimplifyBlock, but even if it was set
                to false, it will get set to true again here because dfachanged
                is true in this case }
              changed := changed or propchanged or dfachanged;

              { If constant propagation made a change but DFA did not, then
                repeating the loop is wasted effort because do_optconstpropagate
                has its own repeat..until loop that only stops when no more
                changes were made, so only check "dfachanged". }
            until not dfachanged;

          { If the node count gets too high, we might have an infinite loop or
            a recursive call that's gone too deep, so break out }
           if changed and (node_count(pureblock) > PURE_NODE_LIMIT) then
            begin
              WritePureNodeLimitWarning;
              PureCleanup;
{$ifdef DEBUG_PURE}
              WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
              WriteLn('Node limit exceeded');
{$endif DEBUG_PURE}
              Exit;
            end;
        ...
        if foreachnode(pm_postprocess, pureblock, @pure_strip_dead_labels, Pointer(pureblock)) then
          { Do one final simplify to get rid of the nothing-nodes that were created }
          if not SimplifyBlock then
            { An error occurred }
            Exit;

        if pureblock.nodetype <> blockn then
          InternalError(2022052801);

{$ifdef DEBUG_PURE}
        WriteLn('--------------------------------');
        WriteLn('Final tree after purity analysis');
        WriteLn('--------------------------------');
  {$ifdef DEBUG_NODE_XML}
        { Has better formatting }
        pureblock.XMLPrintNodeTree(StdOut);
...
{$endif DEBUG_PURE}
        ...
        if (outputs.Count = 0) then
          begin
            ...
            { Check to see if the node tree has collapsed into a single constant write to the result }

            if (
                { More than one statement? }
                Assigned(purestatements.next) and
                (TStatementNode(purestatements.next).statement.nodetype <> exitn) { Permit an exit node }
              ) or
              (purestatements.statement.nodetype <> assignn) or { Statement is not an assignment }
              (tassignmentnode(purestatements.statement).left.nodetype <> loadn) or { Assignment not loading a symbol? }
              (tloadnode(tassignmentnode(purestatements.statement).left).symtableentry.typ<>localvarsym) or { Not the function result? }
              not (vo_is_funcret in tlocalvarsym(tloadnode(tassignmentnode(purestatements.statement).left).symtableentry).varoptions) or
              not is_constnode(tassignmentnode(purestatements.statement).right) then { Not writing a constant to the result? }
              begin
                NonTrivial;
                PureCleanup;
                Exit;
            ...
            { The final node is only the final value of the function result }
            Result := tassignmentnode(purestatements.statement).PruneKeepRight();
          end
        else
          begin
            { This one is more complicated as we have to verify that each and every
              output parameter has been written to, along with the result if
              present }
            ResultCovered := False;
            resultnode := nil;
            outputbody := internalstatements(outputstatements);

            while Assigned(purestatements) do
              begin
                { Permit an exit node if it appears last }
                if (purestatements.statement.nodetype = exitn) then
                  Break;

                if (purestatements.statement.nodetype <> assignn) or { Statement is not an assignment }
                  (tassignmentnode(purestatements.statement).left.nodetype <> loadn) or { Assignment not loading a symbol? }
                  not is_constnode(tassignmentnode(purestatements.statement).right) then { Not writing a constant? }
                  begin
                    NonTrivial;
                    outputbody.Free;
                    PureCleanup;
                ...
                if (tloadnode(tassignmentnode(purestatements.statement).left).symtableentry.typ in [localvarsym, paravarsym]) and
                  (vo_is_funcret in tabstractnormalvarsym(tloadnode(tassignmentnode(purestatements.statement).left).symtableentry).varoptions) then
                  begin
(*                    maybe_create_funcret_node;

                    if not assigned(funcretnode) then
                      { We're writing to the result but there's no function result node? }
                      InternalError(2023081601);
*)
                    ResultCovered := True;

                    { Separate out the actual value and attach to the end of the statement block }
                    resultnode := tassignmentnode(purestatements.statement).PruneKeepRight();
                  end;

                if (tloadnode(tassignmentnode(purestatements.statement).left).symtableentry.typ = paravarsym) then
                  begin
                    { Cycle through the output list to see if we can find the parameter }
                    n := nil;
                    ParameterFound := False;

                    for X := Outputs.Count - 1 downto 0 do
                      begin
                        n := tnode(Outputs[X]);
                        if (tloadnode(tassignmentnode(purestatements.statement).left).symtableentry = tcallparanode(n).parasym) then
                          begin
                    ...
                    if not ParameterFound then
                      begin
                        { We ended up writing to a non-output parameter, which should have been stripped out if this function was pure }
                        NonTrivial;
                        outputbody.Free;
                        PureCleanup;
                        Exit;
                    ...
                    { Create an assignmnet to the actual parameter (use pruning to keep node duplication to a minimum) }
                    n := tcallparanode(n).PruneKeepLeft();

                    { Remove the address taken flag otherwise the variable can't be optimised properly later }
                    Exclude(n.flags, nf_address_taken);

                    { If the result is an implicit parameter, don't set it twice }
                    if not (vo_is_funcret in tabstractnormalvarsym(tloadnode(tassignmentnode(purestatements.statement).left).symtableentry).varoptions) then
                      begin
                        addstatement(
                          outputstatements,
                          cassignmentnode.create(
                            n,
                            tassignmentnode(purestatements.statement).PruneKeepRight()
                          )
                        );
                      end;
                  end
                  ...
                  begin
                    { Writing to something non-local }
                    NonTrivial;
                    outputbody.Free;
                    PureCleanup;
                    Exit;
                ...
                purestatements := tstatementnode(purestatements.next);
              end;

              if (cnf_return_value_used in callnodeflags) then
                begin
                  { If resultnode is nil, ResultCovered will be false and will
                    error out below }
                  if Assigned(resultnode) then
                    { Write to function result }
                    addstatement(outputstatements, resultnode)
                end
              else
                begin
                  { Don't need function result }
                  resultnode.Free;
                  ResultCovered := True;
                end;

            if not ResultCovered or (outputs.Count <> 0) then
              begin
                { Not all the outputs were written to }
                MessagePos2(fileinfo,parser_w_pure_ineligible,tprocdef(procdefinition).procsym.realname,'not all outputs were assigned a value');
                Exclude(procdefinition.procoptions, po_pure);
                outputbody.Free;
                PureCleanup;
{$ifdef DEBUG_PURE}
                WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
                WriteLn('Not all outputs were assigned a value');
{$endif DEBUG_PURE}
                Exit;
              end;

            { Do some final optimisation }
            typecheckpass(outputbody);

            {$ifdef DEBUG_PURE}
            WriteLn('--------------------------------');
            WriteLn('  Complex pure function output  ');
            WriteLn('--------------------------------');
  {$ifdef DEBUG_NODE_XML}
            { Has better formatting }
            outputbody.XMLPrintNodeTree(StdOut);
  {$else DEBUG_NODE_XML}
            outputbody.printnodetree(StdOut);
  {$endif DEBUG_NODE_XML}
{$endif DEBUG_PURE}
    ...
    procedure tcallnode.WritePureNodeLimitWarning;
      begin
        Exclude(callnodeflags, cnf_do_pure);
        Include(callnodeflags, cnf_pure_abort);
        Message(parser_n_pure_analysis_too_complex);
      end;
