     ...
     function DefaultReplacements_GetFPCTarget(param: pointer): ansistring;
       begin
         if (tf_use_8_3 in Source_Info.Flags) or
           ...
           result := target_full_string+'-'+lower(subtarget)
         else
...
{$endif openbsd}
     ...
     procedure DefaultReplacements(var s:ansistring; substitute_env_variables:boolean=true);
     type
       { Linked list of PReplacementNode tracks ongoing DoReplacements calls to prevent recursive expansions of the item into itself:
         "$hello$" occured while expanding an environment variable named "hello" is handled as plain text. }

       PReplacementNode = ^TReplacementNode;
       TReplacementNode = record
         caller: PReplacementNode;
         key: ansistring;
       end;

       function DoReplacements(const s: ansistring; caller: PReplacementNode): ansistring;
         var
           us, envstr: ansistring;
           sp, litStart, dollar2p, ofs: SizeInt;
           envvalue: pchar;

           procedure ReplaceAtSp(nsample: SizeInt; const repl, key: ansistring);
             var
               node: TReplacementNode;
               cc: PReplacementNode;
             begin
               { Check if the item (identified by 'key') appeared while already substituting the same item. }
               cc := caller;
               ...
               node.caller := caller;
               node.key := key;
               DoReplacements := DoReplacements+Copy(s,litStart,sp-litStart)+DoReplacements(repl,@node);
               sp := sp+nsample;
               litStart := sp;
             end;

           function TryReplaceAtSp(const sample: ansistring; getRepl: TGetStringProc; param: pointer=nil): boolean;
             begin
               if us='' then
                 us := Upper(s);
               result := (length(sample)<=length(s)-sp+1) and (CompareChar(sample[1],us[sp],length(sample))=0);
               if result then
                 ReplaceAtSp(length(sample),getRepl(param),sample);
             end;

           function TryReplaceAtSp(const sample: ansistring; const repl: ansistring): boolean;
             begin
               result := TryReplaceAtSp(sample,@DefaultReplacements_GetAnsistring,pointer(repl));
             end;
         ...
         begin
           ...
           repeat
             ...
             if sp<=0 then
               break;
             { Replace some macros }
             if    TryReplaceAtSp('$FPCVERSION',version_string)
                or TryReplaceAtSp('$FPCFULLVERSION',full_version_string)
                or TryReplaceAtSp('$FPCDATE',date_string)
                or TryReplaceAtSp('$FPCCPU',target_cpu_string)
                or TryReplaceAtSp('$FPCOS',target_os_string)
                or TryReplaceAtSp('$FPCBINDIR',exepath)
                or TryReplaceAtSp('$FPCTARGET',@DefaultReplacements_GetFPCTarget)
                or TryReplaceAtSp('$FPCSUBARCH',@DefaultReplacements_GetFPCSubArch)
                or TryReplaceAtSp('$FPCABI',@DefaultReplacements_GetFPCABI)
{$ifdef i8086}
                or TryReplaceAtSp('$FPCMEMORYMODEL',@DefaultReplacements_GetFPCMemoryModel)
{$else i8086}
                or TryReplaceAtSp('$FPCMEMORYMODEL','flat')
{$endif i8086}
{$ifdef mswindows}
                or TryReplaceAtSp('$LOCAL_APPDATA',@DefaultReplacements_GetWindowsSpecialDir,pointer(PtrUint(CSIDL_LOCAL_APPDATA)))
                or TryReplaceAtSp('$APPDATA',@DefaultReplacements_GetWindowsSpecialDir,pointer(PtrUint(CSIDL_APPDATA)))
                or TryReplaceAtSp('$COMMON_APPDATA',@DefaultReplacements_GetWindowsSpecialDir,pointer(PtrUint(CSIDL_COMMON_APPDATA)))
                or TryReplaceAtSp('$PERSONAL',@DefaultReplacements_GetWindowsSpecialDir,pointer(PtrUint(CSIDL_PERSONAL)))
                or TryReplaceAtSp('$PROGRAM_FILES',@DefaultReplacements_GetWindowsSpecialDir,pointer(PtrUint(CSIDL_PROGRAM_FILES)))
                or TryReplaceAtSp('$PROGRAM_FILES_COMMON',@DefaultReplacements_GetWindowsSpecialDir,pointer(PtrUint(CSIDL_PROGRAM_FILES_COMMON)))
                or TryReplaceAtSp('$PROFILE',@DefaultReplacements_GetWindowsSpecialDir,pointer(PtrUint(CSIDL_PROFILE)))
{$endif mswindows}
{$ifdef openbsd}
                or TryReplaceAtSp('$OPENBSD_LOCALBASE',@DefaultReplacements_GetOpenBSDLocalBase)
                or TryReplaceAtSp('$OPENBSD_X11BASE',@DefaultReplacements_GetOpenBSDX11Base)
{$endif}
               then
                 continue;
             ...
             if assigned(envvalue) then
               ReplaceAtSp(dollar2p+length('$')-sp,envvalue,envstr)
             else
           ...
           if litStart=1 then
             exit(s);
           result := result+Copy(s,litStart,length(s)-litStart+1);
         end;

       begin
         s := DoReplacements(s,nil);
       end;
