      ...
      function TryHandleLengthZero(L,R : tnode; op : tnodetype; var resn : tnode) : boolean;
        ...
        begin
          ...
          if op in [equaln,unequaln,gtn,lten] then
            begin
              { “not assigned(lentype)” means than “pointer(L.left) = nil” test is enough. }
              lentype:=nil;
              { COM widestrings always have 32-bit lengths, and also can explicitly have 0 while being non-nil. }
              ...
              { Expand to “(pointer(L.left) = nil) or (PLenType(L.left)[-1] = 0)”, if must. }
              if assigned(lentype) then
                 resn:=caddnode.create_internal(orn,
                     ...
                     cordconstnode.create(0,lentype,false))
                   );
